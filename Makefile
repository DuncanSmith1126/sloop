...
all:
	go get ./pkg/sloop/...
	go fmt ./pkg/sloop/...
	go build -o pkg/plugins/hostrepair/hostrepair.so -buildmode=plugin pkg/plugins/hostrepair/hostrepair.go
	go build -o pkg/plugins/example/example.so -buildmode=plugin pkg/plugins/example/example.go
	go install ./pkg/sloop/...
	go test -cover ./pkg/...
